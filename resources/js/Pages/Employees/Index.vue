...
const openModal = (op,name,email,phone,department,employee) =>{
    ...
    id.value = employee;
    if(op == 1){
        title.value = 'Crear Jugador';
    }else{
        title.value = 'Editar Jugador';
        form.name=name;
        form.email=email;
        form.phone=phone;
...
const deleteEmployee = (id,name) =>{
    ...
    alerta.fire({
        title:'Estas seguro de borrar a: '+name+' ?',
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Jugadores</h2>
        </template>
            ...
            <div class="bg-white grid v-screen place-items-center">
                <div class="mt-3 mb-3 flex">
                   <PrimaryButton @click="$event => openModal(1)">
                       <i class="fa-solid fa-plus-circle"></i>  Crear Juadores
                   </PrimaryButton>
                </div>
            </div>
        ...
        <Modal :show="modal" @close="closeModal">
            ...
            <div class="p-3 mt-6">
                ...
                <TextInput class="mt-1 block w-3/4" v-model="form.name" type="text" id="name" ref="nameInput" placeholder="Name" required></TextInput>
                <InputError :message="form.errors.name" class="mt-2"></InputError>
            </div>
            <div class="p-3">
                <InputLabel for="email" value="Equipo:"></InputLabel>
                <TextInput class="mt-1 block w-3/4" v-model="form.email" type="text" id="email"  placeholder="Email" required></TextInput>
                <InputError :message="form.errors.email" class="mt-2"></InputError>
            </div>
            <div class="p-3">
                <InputLabel for="phone" value="Edad:"></InputLabel>
                <TextInput class="mt-1 block w-3/4" v-model="form.phone" type="text" id="phone" placeholder="Phone" required></TextInput>
            ...
            <div class="p-3">
                ...
                <SelectInput id="department_id"  :options="departments"
                             v-model="form.department_id" type="text" class="mt-1 block w-3/4" required>
                </SelectInput>
                <InputError :message="form.errors.department_id" class="mt-2"></InputError>
            </div>
            <div class="p-3 mt-6 ">
                <PrimaryButton :disabled="form.processing" @click="save">
                    <i class="fa-solid fa-save"></i> Crear Jugador
                </PrimaryButton>
            </div>
